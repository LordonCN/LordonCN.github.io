...
那么准确性又怎么说?如果我想滤传感器传回来的采集值,是不是最后结果显示滤波后与滤波前所画出来的曲线近似完全重合才是最理想的?就像下面这样(我只是看这个效果太好了,没别的意思)<br>
<img src="/img/191122image/2-1lvbo.jpg" >
- 卡尔曼算法中提到的`Q-过程误差`、`R-测量误差`作为实际情况中很关键部分,使得`真实值`既`不等于``测量值`,也不会等于`预测值`.<br>

<img src="/img/191122image/2-2kalman_example.jpg" >
课本中给出的例证也说明了这一点.
...
# 3、C语言实现
下面是伴随`十二十三十四届智能车`走过的简单的卡尔曼函数实现,也见证了两个国一的诞生,那么就先从简单的开始.
```
